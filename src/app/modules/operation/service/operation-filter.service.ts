...
export class OperationFilterService {
  ...
  dateTypeForm$ =
    this.operationFilterFormService.filtersForm.controls.dateType.valueChanges.pipe(
      startWith('insertedAt'),
      shareReplay(1),
    );
  ...
  public getFilteredOperations(initialList$: Observable<Operation[]>) {
    return combineLatest([
      ...
      map(
        ([operations, accountId, quarters, year, filterForm, dateTypeForm]) => {
